...
export function buildThroneworldBoardView(params: BuildBoardViewParams): ThroneworldBoardView {
  ...
  const playerView = game.playerViews
    ? Object.entries(game.playerViews).find(([key]) => key !== "neutral")?.[1]
    : undefined;
...
function buildRenderableSystem(params: BuildRenderableSystemParams): RenderableSystem {
  const { hexId, worldType, position, publicSystem, playerView, playerColors } = params;
  ...
  const resolvedDetails: ThroneworldSystemDetails = publicSystem.details ??
    playerView ?? {
      systemId: hexId,
      dev: 0,
      spaceTech: 0,
  ...
  const owner = resolvedDetails.owner && resolvedDetails.owner !== "neutral" ? resolvedDetails.owner : null;

  const ownerColor = owner ? playerColors[owner] : undefined;

  const scannerColors =
    !isRevealed && publicSystem.scannedBy ? publicSystem.scannedBy.map((p) => playerColors[p]).filter(Boolean) : [];
